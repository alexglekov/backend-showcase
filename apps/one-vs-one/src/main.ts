...
import { OneVsOneModule } from './oneVsOne.module';
import { Config } from './infrastructure/config';

const logger = LoggerModule.loggerFactory();

async function bootstrap() {
  const app = await NestFactory.create(OneVsOneModule, {
    logger,
  });
  ...
  logger.log(`🚀 [${AppsNames.OneVsOne}] server is running on: ${appUrl}`);
}
...
function connectMicroservices(app: INestApplication) {
  ...
  EventsModule.connectDomainEventsListener(app, {
    ...
    groupId: ConsumersGroups.OneVsOneAppConsumer,
  });
}
